...
Take a look at the robots.txt file on the Acme IT Support website to see if they have anything they don't want to list - To do this open Firefox on the AttackBox, and enter the url: [http://10.10.93.144/robots.txt](http://10.10.93.144/robots.txt)[](https://10-10-93-144.reverse-proxy-eu-west-1.tryhackme.com/robots.txt) (_this URL will update 2 minutes from when you start the machine in task 1_)
...
Take a look at the sitemap.xml file on the Acme IT Support website to see if there's any new content we haven't yet discovered: [http://10.10.93.144/sitemap.xml](http://10.10.93.144/sitemap.xml)[](http://10.10.93.144/sitemap.xml) (open this in the FireFox browser on the AttackBox).
...
```shell-session
user@machine$ curl http://10.10.93.144 -v
*   Trying 10.10.93.144:80...
* TCP_NODELAY set
* Connected to 10.10.93.144 (10.10.93.144) port 80 (#0)
> GET / HTTP/1.1
> Host: 10.10.93.144
...
Looking at the page source of our Acme IT Support website ([http://10.10.93.144](http://10.10.93.144/)), you'll see a comment at the end of every page with a page load time and also a link to the framework's website, which is [https://static-labs.tryhackme.cloud/sites/thm-web-framework](https://static-labs.tryhackme.cloud/sites/thm-web-framework). Let's take a look at that website. Viewing the documentation page gives us the path of the framework's administration portal, which gives us a flag if viewed on the Acme IT Support website.
...
```shell-session
user@machine$ ffuf -w /usr/share/wordlists/SecLists/Discovery/Web-Content/common.txt -u http://10.10.174.243/FUZZ
```
...
```shell-session
user@machine$ dirb http://10.10.174.243/ /usr/share/wordlists/SecLists/Discovery/Web-Content/common.txt
```
...
```shell-session
user@machine$ gobuster dir --url http://10.10.174.243/ -w /usr/share/wordlists/SecLists/Discovery/Web-Content/common.txt
```
